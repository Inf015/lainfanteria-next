...
1. **Tabla de decisión de visibilidad** — combiná {sin récords, solo vigentes,
   solo superados, ambos} × {con/sin nacional vigente} × {con/sin fuente} × {con/sin
   auto/lugar/fecha/categoría}. Para cada celda: ¿el bloque aparece?, ¿el
   historial aparece?, ¿la tarjeta muestra el distintivo?, ¿queda alguna línea
   vacía o un separador ` · ` colgando?
2. **CA-1 / CA-8: «idéntica a la de hoy»** — un miembro sin récords no puede
   cambiar ni un nodo del HTML de la ficha ni de la tarjeta. Revisá que no
   quede un `<section>` vacío, un contenedor con margen, o un `0`
   renderizado por `{records.length && …}`.
3. **Enlace de fuente (CA-4)** — solo `http(s)`, `target="_blank"` con
   `rel="noopener noreferrer"`. Buscá cualquier `href` armado sin ese filtro.
4. **Pluralización** — cifra «Récord nacional» / «Récords nacionales»; distintivo
   «RÉCORD NACIONAL» / «2 RÉCORDS NACIONALES». Límites 0, 1, 2.
5. **Server vs Client** — `Records.tsx` debería ser Server Component. Si lleva
   `'use client'` sin necesidad, es S3. Nada de `Date.now()`, `toLocaleString`
   ni zona horaria en render (hidratación, ver `contexto.md`).
6. **Next 16** — `PageProps`, `generateStaticParams`, `next/image` usados como
   dice `node_modules/next/dist/docs/`.
7. **CSS / mobile (CA-9, CA-10)** — no podés ver la página: revisá estáticamente
   `min-width`, anchos fijos en px, `white-space: nowrap` sin `overflow`, grillas
   sin `minmax(0, 1fr)` que desborden a 400 px. Lo no verificable va a *Pruebas
   a ejecutar por el PM*.
8. **Accesibilidad** — el distintivo tiene texto real; emoji con
   `aria-hidden`; jerarquía de títulos (`h2` del bloque, sin saltar a `h4`).
9. **Alcance** — el diff **no** puede tocar `app/(admin)/`, `lib/records.ts`,
   `lib/types.ts` ni `lib/datos.ts`. Si los toca: S2/P1 (rompe el paralelismo
   con T-002).
10. **Evidencia visual** — si la entrega marca CA visuales sin haber levantado
    el entorno local con los datos A/B/C del brief, es defecto de proceso (S3/P2).
